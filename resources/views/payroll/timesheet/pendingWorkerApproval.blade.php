    ...
    <section id="NeedsApproval">
        <div class="row">
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    {!! Form::open(['route' => 'payroll.timesheet.setfilter', 'class'=>'form-inline'])!!}
                        ...
                        &nbsp;&nbsp;<a href="{{ route('payroll.timesheet.resetfilter') }}?page=pendingWorkerApproval" class="btn btn-danger btn-xs"> <i class="fa fa-times"></i> Clear</a>
                    {!! Form::close() !!}
                    </div>
                </div>
            </div><br>
            ...
            <div class="col-md-12 col-lg-12 col-xs-12">
                ...
                ?>
                @if($client && $worker)
                {!! Form::open(['route' => 'payroll.timesheet.submitPendingWorkerApproval','autocomplete' => 'off']) !!}
                <div class="panel panel-default Worker-{{$worker->id}} Timecards">
                    <div class="panel-heading" role="tab" id="heading{{$timecard->id}}">
                        ...
                        <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse{{$timecard->id}}" aria-expanded="true" aria-controls="collapse{{$timecard->id}}">
                            <h4><i class="fa fa-plus-square"></i> Client: {{$client->client_name}} , Worker: {{$worker->fullname}} , Billing Cycle Ending: {{date('m/d/y',strtotime($timecard->end_date))}}</h4>
                        </a>
                        </div>
                            ...
                            <div class="row" style="padding: 10px 0px 40px 0px;overflow-x: scroll;">
                                <div class="box-body table_group">
                                    ...
                                    <tr>
                                        ...
                                        <td >
                                            <a href="#" class="btn btn-success" data-toggle="modal" data-target="#modal-approve-hours-{{$timecard->id}}"><i class="fa fa-check"></i> Approve Hours</a>
...
$('.search_clients').change(function () {
    ...
    $.get("{{route('payroll.client.allWorkers')}}",{"clients_id":id, "from": "pending_worker"}).done(function( workers ) {
