    ...
    <section id="PayrollImmediatePay">
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" >
                        <h3 class="bold">Payments - Immediate Pay (Veem)</h3>
                    </div>
                </div>
            </div>
                    ...
                    <div class="box-body table_group" style="overflow-x:scroll">
                        ...
                        <tr class="payment_{{$payment->id}}" style="{{$style}}">
                            <td>
                                ...
                                @component('admin.payroll.components.paymodal')
                                @slot('id') 
                                    {{ $payment->id }}
                                @endslot
                                @slot('from') 
                            ...
                            <td>{{$payment->client() ? $payment->client()->client_name : ''}}</td>
                            ...
                            <td><a href="{{route('admin.payroll.viewReport',$payment->id)}}" target="_blank">View Report</a></td>
                            <td><input class="comments form-control" data="{{$payment->id}}" value="{{$payment->comments}}"/></td>
                            <td>
                                <a href="/admin/editPayment/{{$payment->id}}?from=immediateVeem" class="btn btn-success btn-xs"><i class="fa fa-edit"></i></a>
                                ...
                                @component('admin.payroll.components.removepayment')
                                @slot('id') 
                                    {{ $payment->id }}
                                @endslot
                                @slot('from') 
                                ...
                                @component('admin.payroll.components.changemodal')
                                @slot('id') 
                                    {{ $payment->id }}
                                @endslot
                                @slot('from') 
...
function sendSummary(e){
    var pid=$(e).attr('data');
    $.get("{{route('admin.payroll.sendSummary')}}",{'id':pid}).done(function(data){
        if(data=="success"){
...
$('.comments').change(function(){
    ...
    $.get("{{route('admin.updateCommets')}}",{'pid':pid,'comments':comments,'type':'payment'}).done(function(data){
    });
});
