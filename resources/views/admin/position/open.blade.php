...
<section id="OpenPositions">
    <div class="row">
        ...
        <div class="col-md-12 col-lg-12 col-xs-12 AccountManager-{{$accountManager->id}} AccountManagers">
            ...
            <?php
                $assignedWorkerList = $accountManager->candidateWorkers()->get()->pluck('full_name', 'id')->prepend('','')->toArray();
            ?>
                        ...
                        <div class="panel-title ">
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse{{$position->id}}" aria-expanded="true" aria-controls="collapse{{$position->id}}">
                                <h4>
                                    ...
                                    <a href="{{route('admin.position.copy', $position->id)}}" class="btn btn-primary pull-right btn-xs">Copy</a> &nbsp;
                                    <a href="{{route('admin.position.edit', $position->id)}}?from=edit" class="btn btn-warning pull-right btn-xs">Edit</a>
                                </h4>
                            </a>
                        </div>
                            ...
                            <div class="row" style="margin-left: 10px; padding-top: 20px">
                                ...
                                <div class="col-xs-12 group-{{$group->id}}">
                                    <div class="panel panel-default">
                                        ...
                                        <div class="panel-body">
                                                ...
                                                <table id="detail_table" class="table text-center table-bordered table-striped">
                                                    ...
                                                    <tbody class="tbody-candidates" group_id="{{$group->id}}">
                                                        ...
                                                        <tr>
                                                            ...
                                                            ?>
                                                            <td class="{{$column['field'] == 'name' ? 'headcol' : 'other-col'}}">
                                                                @if($column['field']=='name' && $column['field_type']=='dropdown')
                                                                {!!  Form::select('candidate_name', $assignedWorkerList, $value, ['class' => 'form-control name candidate_name', 'onchange'=>'updateCandidateValues(this, `'.$columns.'`)']) !!}
                                                                ...
                                                                <a type="hyperlink" class="btn {{$column['field']}}" href="{{str_replace('accountManager', 'admin',str_replace('www.', '', $value))}}" target="_blank" oncontextmenu="editHyperlink(this, event, '{{$column['field']}}', '{{$columns}}')">Click Here</a>
                                            ...
                                            <button type="button" class="btn btn-warning btn-add-candidate" onclick="add_candidate(this, '{{json_encode($position->columns())}}', '{{$group->id}}', '{{$position->id}}', '{{$accountManager->id}}')" style="margin-top: 10px"><i class="fa fa-plus"></i> Add Row</button>
                                        </div>
                                    </div>
                                </div>
                                @endif
                                ...
                                <div class="col-xs-12 group-completed hidden">
                                    <div class="panel panel-default">
                                        ...
                                        <div class="panel-body">
                                            <?php $count = count($position->completedCandidates()); $height = $count>=5 ? 350: (100+51*$count); ?>
                                            <div class="box-body table_group scroll" style="overflow: scroll; height: {{$height}}px; padding:0px">
                                                <table id="detail_table" class="table text-center table-bordered table-striped">
                                                    <thead>
                                                    <tr>
        ...
        @endforeach
        <div class="col-md-12 col-lg-12 col-xs-12 AccountManager-unassigned AccountManagers">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <h4 class="bold">- Unassigned Clients</h4>
                    <br>
                </div>
            </div>
            @foreach(\App\Clients::where('account_managers_id', null)->orderBy('client_name')->get() as $client)
            @if ($client->positions()->where('status','active')->count('id')>0)
            <div class="Client-{{$client->id}} Clients">
                @foreach($client->positions()->where('status','active')->orderBy('name')->get() as $position)
                <div class="panel panel-default Position-{{$position->id}} Positions">
                                                ...
                                                <table id="detail_table" class="table text-center table-bordered table-striped">
                                                    ...
                                                    <tbody class="tbody-candidates" group_id="{{$group->id}}">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="{{$column['field'] == 'name' ? 'headcol' : ''}}">
                                                                @if($column['field']=='name' && $column['field_type']=='dropdown')
                                                                {!!  Form::select('candidate_name', $unassignedWorkerList, $value, ['class' => 'form-control name candidate_name', 'onchange'=>'updateCandidateValues(this, `'.$columns.'`)']) !!}
                                                                @elseif($column['field_type']=='dropdown')
                                                                    @if(isset($column['drop_down_options']))
                                                                    <?php 
                                                                        $optionList=explode(',', $column['drop_down_options']);
...
$('.search_account').change(function() {
    const account_id=$('.search_account').val();
...
var workers={!! $workers !!};
function generateNameSelectBox(columns, accountManageId) {
    ...
    workers.forEach((worker) => {
        if (worker.candidate_account_manager_id == accountManageId) {
...
function add_candidate(e, columns, groupId, positionId, accountManageId = 0) {
    ...
    cols.forEach((col) => {
        ...
        if (col.field == 'name' && col.field_type=='dropdown') {
            element = generateNameSelectBox(colList, accountManageId);
...
function remove_candidate(e) {
    ...
    $.ajax({
        url: "{{ url('/admin/canditate/remove') }}",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "post",
...
function finalDecisionCandidate() {
    ...
    if (!candidate_id) return;
    $.ajax({
        url: "{{ url('/admin/canditate/final-decision') }}",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "post",
...
function moveToNewGroup(position_id) {
    ...
    if (!candidate_id) return;
    $.ajax({
        url: "{{ url('/admin/canditate/move-to-group') }}",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "post",
...
function saveNote() {
    ...
    $.ajax({
        url: "{{ url('/admin/canditate/update-note') }}",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "post",
...
function updateOrder(el) {
    ...
    $.ajax({
        url: "{{ url('/admin/canditate/update-order') }}",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "post",
