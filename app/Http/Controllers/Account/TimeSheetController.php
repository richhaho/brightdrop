...
class TimeSheetController extends Controller
{
    ...
    public function pendingWorkerApproval()
    {
        ...
        $allClients = $accountmanager->clients()->where('status', 'active')->orderBy('client_name', 'ASC')->get();
        ...
        return view('account.timesheet.pendingWorkerApproval',$data);
    }
    ...
    public function needsApproval()
    {
        ...
        return view('account.timesheet.needsApproval',$data);
    }
    public function submitApprove(Request $request)
    {
        ...
        if ($client->pto_infomation!='yes' || $pto_days_worker_default==0) {
            if ($total_pto_time>0){
                ...
                return redirect()->route('account.timesheet.needsApproval');
            }
        }
    ...
    {
        $accountmanager=Auth::user()->accountmanager();
        $search_clients=$accountmanager->clients()->orderBy('client_name', 'ASC')->get()->pluck('client_name','id')->prepend('','');
        ...
        return view('account.timesheet.current',$data);
    }

    public function pastTimesheets()
    {
        $accountmanager=Auth::user()->accountmanager();
        $search_clients=$accountmanager->clients()->where('status', 'active')->orderBy('client_name', 'ASC')->get()->pluck('client_name','id')->prepend('','');
        ...
        return view('account.timesheet.past',$data);
    }
    public function setfilter (Request $request) {
        ...
        return redirect()->route('account.timesheet.'.$page);
    }
    ...
    public function submitCurrent(Request $request)
    {
        ...
        if ($client->pto_infomation!='yes' || $pto_days_worker_default==0) {
            if ($total_pto_time>0){
                Session::flash('message', 'Your time card has some PTO hours that client did not set. You can not use PTO time for this client.');  
                return redirect()->route('account.timesheet.currentTimesheets');
            }
        }
        $pto_remain=$pto_days_worker_default*8-$pto_sum;
        if ($pto_remain<$total_pto_time && $total_pto_time>0){
        ...
        Session::flash('message', $worker->fullname()."'s time card was saved for client:".$client->client_name."."); 
        return redirect()->route('account.timesheet.currentTimesheets');
    }
    public function submitPendingWorkerApproval(Request $request)
    {
        ...
        if ($client->pto_infomation!='yes' || $pto_days_worker_default==0) {
            if ($total_pto_time>0){
                Session::flash('message', 'Your time card has some PTO hours that client did not set. You can not use PTO time for this client.');  
                return redirect()->route('account.timesheet.pendingWorkerApproval');
            }
        }
        $pto_remain=$pto_days_worker_default*8-$pto_sum;
        if ($pto_remain<$total_pto_time && $total_pto_time>0){
    ...
    public function sendEmailApprovedTimecard($timecard){
        ...
        Mail::to($mailto)->send(new TimecardApprovedWorker($timecard,'Account Manager('.Auth::user()->accountmanager()->fullname.')'));
    }
    ...
    public function movetoQueue_invoice($timecard){
        ...
        foreach ($timecards as $timecard) {
            ...
            //$regular_time=$client->overtime_pay_provided=='yes' ? (80-$ptoHours-$holidayHours) : $total_work_time;
